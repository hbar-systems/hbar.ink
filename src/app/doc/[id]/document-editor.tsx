...
function DocumentEditorContent({ document: docRow }: { document: DocType }) {
  ...
  useEffect(() => {
    ...
    if (savedFont) {
      setFontOverride(savedFont)
    }
  }, [])

  // Save editor width preference to localStorage
  ...
  // Get font class based on override or preset
  const getFontClass = () => {
    ...
    // Default: Montserrat for both presets (user can override)
    return 'font-montserrat'
  }

  // Load local draft if it exists and is newer
  // Disabled to prevent SSR issues - autosave still works
  // useEffect(() => {
  ...
  //         const shouldRestore = window.confirm('A more recent local draft was found. Would you like to restore it?')
  //         if (shouldRestore) {
  ...
  // Save document when title changes (debounced)
  useEffect(() => {
    if (title === docRow.title) return // Don't save if unchanged
    
    const titleTimeout = setTimeout(async () => {
      await saveDocument()
    }, 500) // Faster for title changes
  ...
  const saveDocument = async () => {
    ...
    try {
      ...
      // Only set lastSaved after confirmed persistence
  ...
  const handleExport = () => {
    ...
    // Format: hbarink__{title-or-untitled}__{yyyy-mm-dd}.md
  ...
  const handleDuplicateAsDraft = async () => {
    try {
      ...
      if (!user) {
        showToast('You must be logged in to duplicate a document', 'error')
        return
      }
      ...
      if (data) {
        // Immediately redirect to the new draft
  ...
  const handleConfirmSeal = async () => {
    if (status !== 'terminal') {
      ...
      try {
        // Try to update with sealed_at, but fall back if column doesn't exist
        const updatePayload: any = { status: 'terminal' }
        
        // Only add sealed_at if the column exists (check by trying)
        const { error } = await supabase
          .from('documents')
        ...
        if (error) {
          ...
          showToast(`Failed to seal document: ${error.message}`, 'error')
          return
        }
  ...
  return (
    <>
      <div className={`${stylePreset} min-h-screen`} style={{ backgroundColor: '#f6f5f2' }}>
        {/* Header hover detection zone (always active in focus mode) */}
        {focusMode && (
          <div 
        ...
        <header 
          className={`border-b border-gray-200 transition-opacity duration-300 relative z-50 ${
                ...
                <div className="flex items-center gap-3 text-xs text-gray-500">
                  ...
                  <button
                    ...
                    className={`transition-colors ${
                      editorWidth === 'narrow' ? 'text-gray-900' : 'text-gray-400 hover:text-gray-600'
                    }`}
                  >
                    Narrow
                  </button>
                  <button
                    onClick={() => handleWidthChange('comfort')}
                    className={`transition-colors ${
                      editorWidth === 'comfort' ? 'text-gray-900' : 'text-gray-400 hover:text-gray-600'
                    }`}
                  >
                    Comfort
                  </button>
                  <button
                    ...
                    className={`transition-colors ${
                      editorWidth === 'wide' ? 'text-gray-900' : 'text-gray-400 hover:text-gray-600'
                    }`}
                  >
                    Wide
                  </button>
              ...
              <div className="flex items-center gap-4">
                ...
                <button
                  ...
                  className={`px-3 py-1.5 text-xs transition-colors ${
                    focusMode ? 'text-gray-900 font-medium' : 'text-gray-600 hover:text-gray-900'
                  }`}
                >
                  Focus
                </button>
                  ...
                  {showExportMenu && (
                    <div className="absolute right-0 mt-1 bg-white border border-gray-200 shadow-lg rounded z-50 min-w-[120px]">
                      ...
                      <button
                        ...
                        className="block w-full text-left px-4 py-2 text-xs text-gray-700 hover:bg-gray-50 transition-colors border-t border-gray-100"
                      >
                        .pdf (PDF)
                      </button>
                    </div>
                ...
                {status === 'terminal' ? (
                  <button
                    ...
                    className="px-4 py-1.5 text-xs text-gray-900 border border-gray-300 hover:border-gray-400 transition-colors"
                  >
                    Duplicate to Edit
                  </button>
                ) : (
        ...
        <div className={`mx-auto transition-all duration-300 ${
          ...
          lineHeight: stylePreset === 'WritersRoom' ? '1.75' : '1.75'
        }}>
                ...
                {status === 'terminal' ? (
                  ...
                  <>
                    <select
                      ...
                      className="text-xs border-0 bg-transparent focus:ring-0 text-gray-500 p-0"
                    >
          ...
          {/* Writing area - no card, no borders */}
          <div className="px-8 pb-24">
            {showPreview ? (
              <div className={`prose prose-lg max-w-none ${
                stylePreset === 'NightInk' ? 'prose-invert' : ''
              } ${getFontClass()}`}>
              ...
              <textarea
                ...
                className={`w-full min-h-[70vh] leading-relaxed resize-none focus:outline-none border-none bg-transparent ${
                  stylePreset === 'NightInk' ? 'text-gray-100 placeholder-gray-600' : 'text-gray-800 placeholder-gray-300'
                } ${getFontClass()}`}
